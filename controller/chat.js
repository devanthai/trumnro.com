...
router.post('/', checklogin, async (req, res) => {
    ...
    if (!req.user.isLogin) {
        ...
        // const newChat = new Chat({ vip: vipp, top: index + 1, clan: typeclan, server: server, name: req.user.name, noidung: noidung, sodu: req.user.vang, type: type, uid: req.user._id, token: token, avatar: req.user.avatar })
        try {
            // const saveChat = await newChat.save()
            //console.log(req.user.vang)
