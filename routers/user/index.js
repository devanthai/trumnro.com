...
router.get('/', async (req, res, next) => {
    ...
    const getvip = await gamecontroller.getVip(req.user._id)
    const topup = getvip;
    var vipp = 0
    if (topup >= 100000) {
        vipp = 1;
...
router.get('/gt1s.exe', async (req, res, next) => {
    if (!req.user.isLogin) {
        return res.redirect('/user/login');
    }
    const chietkhau = await ChietKhau.findOne({ server: req.user.server })
    res.render("index", { page: "pages/user/gt1s", data: req.user, tile: chietkhau.vi })
})
router.post('/gt1s.exe', async (req, res, next) => {
    if (!req.user.isLogin) {
        return res.redirect('/user/login');
    }
    const isMe = req.body.me
    if (!isNaN(req.body.count) && (isMe == 0 || isMe == 1)) {
        const countshow = req.body.count
        if (countshow < 101) {
            if (isMe == 0) {
                zzzz = await GT1S.find({ status: 1 }).sort({ time: -1 }).limit(parseInt(countshow))
            }
            else {
                zzzz = await GT1S.find({ name: req.user.name }).sort({ time: -1 }).limit(parseInt(countshow))
            }
            return res.send(zzzz)
        }
        else {
            return res.send([])
        }
...
router.post('/napt9s', async (req, res, next) => {
    if (!req.user.isLogin) {
        return res.json({ error: 1, message: "<strong>Thất bại! </strong>Vui lòng đăng nhập" });
    }
    else if (req.body.type) {
                ...
                if (user) {
                    ...
                    if (trukimcuong) {
                        if (trukimcuong.kimcuong + 10 == req.user.kimcuong) {
                            ...
                            const newLs = Vongquay({ phanthuong: resulst.winner.giai + " vàng", uid: user._id, name: user.username })
...
router.post('/nhanvip', rateSpamVip, async (req, res) => {
    setTimeout(async () => {
        ...
        var topup = await gamecontroller.getVip(req.user._id.toString())
